...
## Full-Context as Conversational Memory Baseline

In [[Conversational Memory]] evaluation, the full-context approach — passing the entire conversation history within the LLM's context window — represents the ICL upper bound. On the [[LOCOMO Benchmark]], full-context processing achieves the highest [[LLM-as-a-Judge]] score (72.90%) among all methods, but at extreme cost: p95 total latency of 17.1 seconds and ~26,000 tokens consumed per query. This makes it impractical for production deployments, motivating memory-based approaches like [[Mem0]] that achieve 91% lower p95 latency while maintaining competitive accuracy.
